    ...
    fn int_width(&self) -> usize {
        match self {
            Self::Range(min, max) => match () {
                ...
                _ if *min >= u32::MIN as isize && *max <= u32::MAX as isize => 32,
                _ if *min >= u64::MIN as isize && *max <= u64::MAX as isize => 64,
                _ if *min >= u128::MIN as isize && *max <= u128::MAX as isize => 128,
                _ if *min >= i8::MIN as isize && *max <= i8::MAX as isize => 8,
                _ if *min >= i16::MIN as isize && *max <= i16::MAX as isize => 16,
                _ if *min >= i32::MIN as isize && *max <= i32::MAX as isize => 32,
                _ if *min >= i64::MIN as isize && *max <= i64::MAX as isize => 64,
                _ if *min >= i128::MIN as isize && *max <= i128::MAX as isize => 128,
    ...
    fn resolve_type(&mut self, type_: &ast::Type) -> Type {
        match type_ {
            ast::Type::Named(name) => match name.0.as_ref() {
                ...
                "u32" => Type::Range(u32::MIN as isize, u32::MAX as isize),
                "u64" => Type::Range(u64::MIN as isize, u64::MAX as isize),
                "u128" => Type::Range(u128::MIN as isize, u128::MAX as isize),
                "i8" => Type::Range(i8::MIN as isize, i8::MAX as isize),
                "i16" => Type::Range(i16::MIN as isize, i16::MAX as isize),
                "i32" => Type::Range(i32::MIN as isize, i32::MAX as isize),
                "i64" => Type::Range(i64::MIN as isize, i64::MAX as isize),
                "i128" => Type::Range(i128::MIN as isize, i128::MAX as isize),
                "bool" => Type::Boolean,
