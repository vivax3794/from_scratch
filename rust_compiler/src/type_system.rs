...
impl TypedExpression {
    ...
    fn generic(self) -> (Type, ir::Expression) {
        match self {
            ...
            Self::Bool(expr) => {
                (Type::Boolean, ir::Expression::Bool(expr))
            }
        }
    }
...
impl TypeNarrows {
    fn narrow(
        ...
        let TypedExpression::Int(left_range, left_expr) = left_expr
        else {
            return Self(HashMap::new());
        };
        let TypedExpression::Int(right_range, right_expr) =
            right_expr
        else {
            return Self(HashMap::new());
        };
    ...
    fn resolve_statement(
        ...
        match stmt {
            ...
            ast::Statement::Return(expr) => {
                ...
                let (expr_type, expr) =
                    convert_if_possible(expr, &return_type).generic();
            ...
            ast::Statement::VaribleBinding {
                ...
                let (value_type, value) =
                    convert_if_possible(value, &type_).generic();
    ...
    fn resolve_expression(
        ...
        match expr {
            ...
            ast::Expression::Comparison(left_ast, chains) => {
                ...
                while index < exprs.len() - 1 {
                    ...
                    narrow = narrow.narrow(
                        *comparison,
                        (left_expr.clone(), *left_ast.clone()),
                        (right_expr.clone(), *right_ast.clone()),
                    );
            ...
            ast::Expression::Binary(left, op, right) => {
                let (left_range, left) =
                    self.resolve_expression(left).0.int();
                let (right_range, right) =
                    self.resolve_expression(right).0.int();
